...
if (isset($_POST['addRental'])) {
    ...
    $stmt = $conn->prepare("SELECT ilosc FROM ksiazka_MM WHERE id = ?");
    ...
    if (!$ksiazka) {
        ...
        // Додаємо позичання.
        $stmt = $conn->prepare("INSERT INTO wypozyczenie_MM (czytelnik_id, ksiazka_id, ilosc) VALUES (?, ?, ?)");
        $stmt->bind_param("iii", $czytelnik_id, $ksiazka_id, $ilosc_pozyczona);
        if ($stmt->execute()) {
            // Оновлюємо кількість книг у базі даних.
            $stmt = $conn->prepare("UPDATE ksiazka_MM SET ilosc = ilosc - ? WHERE id = ?");
    ...
    <main>
        ...
        <form method="POST">
            ...
            <select id="czytelnik_nazwisko" name="czytelnik_nazwisko" required>
                ...
                <?php
                $result = $conn->query("SELECT id, nazwisko FROM czytelnik_MM");
            ...
            <select id="ksiazka_tytul" name="ksiazka_tytul" required>
                ...
                <?php
                $result = $conn->query("SELECT id, tytul FROM ksiazka_MM");
                while ($row = $result->fetch_assoc()) {
