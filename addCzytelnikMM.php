...
<body>
    ...
    <main>
        ...
        if (isset($_POST['addReader'])) {
            ...
            $stmt = $conn->prepare("SELECT id FROM ulica_MM WHERE nazwa = ?");
            ...
            if ($row = $result->fetch_assoc()) {
                ...
                $stmt = $conn->prepare("INSERT INTO ulica_MM (nazwa) VALUES (?)");
                $stmt->bind_param("s", $ulica);
                if (!$stmt->execute()) {
            ...
            // Додавання читача
            $stmt = $conn->prepare("INSERT INTO czytelnik_MM (nazwisko, ulica_id, ulica_numer, mieszkanie_numer) VALUES (?, ?, ?, ?)");
